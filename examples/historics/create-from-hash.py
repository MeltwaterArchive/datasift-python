...
def usage(message = '', exit = True):
    ...
    if len(message) > 0:
        sys.stderr.write('\n%s\n' % message)
    sys.stderr.write('\n');
    sys.stderr.write('Usage: create-from-hash.py <username> <api_key> \\\n')
    sys.stderr.write('             <stream_hash> <start_date> <end_date> <sources> <sample> <name>\n')
    sys.stderr.write('\n')
    sys.stderr.write('Where: stream_hash = the stream hash the Historics query should run\n')
